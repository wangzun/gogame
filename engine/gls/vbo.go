...
type VBO struct {
	...
	handle  gl.Buffer       // OpenGL handle for this VBO
...
func (vbo *VBO) Transfer(gs *GLS) {
	...
	if vbo.gs == nil {
		...
		for _, attrib := range vbo.attribs {
			...
			gs.VertexAttribPointer(loc, attrib.NumElements, attrib.ElementType, false, int32(strideSize), attrib.ByteOffset)
		}
	...
	gs.BindBuffer(ARRAY_BUFFER, vbo.handle)
	gs.BufferData(ARRAY_BUFFER, vbo.buffer.Bytes(), f32.Bytes(binary.LittleEndian, vbo.buffer...), vbo.usage)
	vbo.update = false
}
