...
type Program struct {
	...
	handle   gl.Program       // OpenGL program handle
...
type shaderInfo struct {
	...
	handle gl.Shader // OpenGL shader handle
}
...
func (prog *Program) Handle() gl.Program {
...
func (prog *Program) AddShader(stype uint32, source string) {
	...
	prog.shaders = append(prog.shaders, shaderInfo{stype, source, gl.Shader{Value: 0}})
}
...
func (prog *Program) DeleteShaders() {
	...
	for _, shaderInfo := range prog.shaders {
		if shaderInfo.handle.Value != 0 {
			prog.gs.DeleteShader(shaderInfo.handle)
			shaderInfo.handle.Value = 0
		}
	}
}
...
func (prog *Program) GetUniformLocation(name string) int32 {
	...
	locUniform := prog.gs.GetUniformLocation(prog.handle, name)
	prog.gs.stats.UnilocMiss++

	// Cache result
	loc = locUniform.Value
	prog.uniforms[name] = locUniform.Value
	if loc < 0 {
...
func (prog *Program) CompileShader(stype uint32, source string) (gl.Shader, error) {

	// Create shader object
	shader := prog.gs.CreateShader(stype)
	if shader.Value == 0 {
		return gl.Shader{Value: 0}, fmt.Errorf("error creating shader")
	}
