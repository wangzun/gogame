...
func (m *Physical) RenderSetup(gs *gls.GLS) {

	m.Material.RenderSetup(gs)
	location := m.uni.Location(gs)
	data := []float32{
		m.udata.baseColorFactor.R,
		m.udata.baseColorFactor.G,
		m.udata.baseColorFactor.B,
		m.udata.baseColorFactor.A,
		m.udata.emissiveFactor.R,
		m.udata.emissiveFactor.G,
		m.udata.emissiveFactor.B,
		m.udata.emissiveFactor.A,
		m.udata.metallicFactor,
